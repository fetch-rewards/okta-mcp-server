...
class OktaAuthManager:
    ...
    def __init__(self):
        ...
        # Scope the keyring service name to the org so prod and preview tokens
        # never overwrite each other when both servers run simultaneously.
        org_host = self.org_url.removeprefix("https://").rstrip("/")
        self._service_name = f"OktaAuthManager:{org_host}"
